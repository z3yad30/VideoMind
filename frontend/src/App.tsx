        ...
        {videoId && (
          <>
            ...
            <div className="content-grid">
              ...
              <section className="detail-surface panel">
                <section className="summary"><div className="panel-heading compact"><div><span className="section-kicker">Video detail</span><h2>Summary</h2></div><span className="summary-icon">✦</span></div>{summary ? <div className="summary-body">{Object.entries(summary).map(([key, value]) => <div className="summary-block" key={key}><span>{key.replaceAll("_", " ")}</span><p>{value}</p></div>)}</div> : <EmptyState text={isReady ? "Summary is not available for this source." : "A concise summary will appear here when processing finishes."} />}{summary && <SummaryAudio videoId={videoId} />}</section>
                <section className="transcript"><div className="panel-heading compact"><div><span className="section-kicker">Source record</span><h2>Transcript</h2></div>{transcript.length > 0 && <span className="count-label">{transcript.length} segments</span>}</div>{transcript.length ? <div className="transcript-list">{transcript.map((segment, index) => <button className="transcript-row" type="button" key={`${segment.start}-${index}`} onClick={() => jumpTo(segment.start)} disabled={!previewUrl}><span>{formatTime(segment.start)}</span><p>{segment.text}</p></button>)}</div> : <EmptyState text={isReady ? "No transcript is available for this source." : "Your timestamped transcript will appear here when processing finishes."} />}</section>
              </section>
            </div>
            <section className="ask panel"><div className="panel-heading compact"><div><span className="section-kicker">Ask the video</span><h2>What do you want to know?</h2></div></div><div className="ask-box"><textarea value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={submitOnEnter} disabled={!canAsk || isAsking} placeholder={canAsk ? "Ask about a moment, idea, or detail..." : "Available when processing finishes"} rows={3} /><div className="ask-actions"><span>Enter to ask · Shift + Enter for a new line</span><button className={`mic-button ${isRecording ? "recording" : ""}`} type="button" onClick={() => void toggleRecording()} disabled={!canAsk || isAsking} title={isRecording ? "Stop recording" : "Ask with your microphone"}>{isRecording ? "■" : "●"}</button><button className="ask-button" type="button" onClick={() => void ask()} disabled={!canAsk || !question.trim() || isAsking}>{isAsking ? "Thinking..." : "Ask"}<span>↗</span></button></div></div>{voiceQuestion && <div className="voice-question"><span>Heard you say</span><p>“{voiceQuestion}”</p></div>}{answer && <AnswerCard answer={answer} onJump={jumpTo} canJump={Boolean(previewUrl)} videoId={videoId} />}</section>
          </>
        )}
      </section>
